...
async function migrateRecipes(fileName) {
  ...
  for (const recipe of recipes) {
    const ingredients = recipe.ingredients ? splitMarkdownChunks(recipe.ingredients) : []
    ...
    const values = {
      ...
      category: recipe.category ? { connect: { slug: recipe.category.slug } } : null,
      seo: {
        create: {
          htmlTitle: recipe.seo ? recipe.seo.htmlTitle : recipe.title,
          htmlDescription: recipe.seo ? recipe.seo.htmlDescription : (recipe.headline ?? recipe.title)
        }
      }
    }
    ...
    console.log(recipe.ingredients ? splitMarkdownChunks(recipe.ingredients).length : '')
    // console.log(recipe.ingredients)
    ...
    if (data.recipe === null) {
      ...
      const { data: createData, errors: createErrors, ...rest } = await createRecipe(values)
      if (createErrors) {
      ...
      const { data: updateData, errors: updateErrors } = await updateRecipe(data.recipe.id, values)
